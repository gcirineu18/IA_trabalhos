...
6. A representação do nó de busca será a classe **`Node`**, que conterá os atributos `father`, `current_state`, `visited`, `direction` e `depth`
...
- Observou-se a profundidade do estado objetivo com o BFS é quase sempre menor do que a profundidade obtida pelo DFS, o que é natural, já que é da propriedade do algoritmo DFS sempre explorar ao máximo a profundidade de um nó. Porém, tanto o consumo de memória quanto o tempo de execução variam muito já que irá depender da posição mais relevante na busca do estado objetivo na árvore, se ela se encontra nos extremos ou mais centralizada.

- Como os pesos das arestas não são relevantes para o problema, a questão de otimalidade não é levada em conta, apenas a estrutura de dados (Pilha ou Fila) escolhida é considerada como distinção principal.
