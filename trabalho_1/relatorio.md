...
- Será gerado um arquivo `dfs.txt` ou `bfs.txt` a depender do algortimo escolhido. Nele vão haver informações como o tempo de execução, o número de nós visitados, o número de ancestrais e a exibição deles (de baixo para cima) até chegar na resolução e o sentido escolhido.
--- 
...
6. A representação do nó de busca será a classe **`Ǹode`**, que conterá os atributos `father`, `current_state`, `visited`, `direction`
7. O algoritmo utiliza a estrutra de dados *`Set`* para controlar que cada nó com um dado estado só pode ser armazenado uma vez, e impedir uma complexidade exponencial.

### Análise comparitiva

- Observou-se a profundidade do estado objetivo com o BFS é quase sempre menor do que a produndidade obtida pelo DFS, o que é natural, já que é da propriedade do algoritmo DFS encontrar sempre explorar ao máximo a profundidade de um nó. Porém, tanto o consumo de memória quanto o tempo de execução variam muito já que irá depender da posição mais relevante na busca do estado objetivo na árvore, se ela se encontra nos extremos ou mais centralizada.
...
O algoritmo executa em tempo `O(V + E)` com memoização aplicada, considerando V os vértices (nós), e E as arestas (vizinhos) de cada vértice.
